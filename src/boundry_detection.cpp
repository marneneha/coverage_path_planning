...
void boundry_detection_node_class::onInit(){
    ...
    update_map_service = nh.serviceClient<coverage_planning::UpdateMap>("/uav1/coverage_planning_boustrophedon/update_map_service");
...
void boundry_detection_node_class::pixel23D(){
    ...
    for(int i = 0; i< pixel_boundry_vector.size(); i++){
        ...
        ground_waypoint.position.x = pt3d.x*altitude;
        ground_waypoint.position.y = pt3d.y*altitude;
        ground_waypoint.position.z = pt3d.z*altitude;
    ...
    boundry_detection_node_class::TFBroadcaster();
...
void boundry_detection_node_class::waypoint_wrapper(){ 
    ...
    for(int i = 0; i < waypoint_iterator_vector.size(); i++){
        std::cout<<"x is"<<ground_waypoint_vector[i].position.x<<"y is"<<ground_waypoint_vector[i].position.y<<"waypoint_iterator_vector[i] is"<<waypoint_iterator_vector[i]<<std::endl;
        waypoint.position = ground_waypoint_vector[waypoint_iterator_vector[i]].position;
            std::cout<<"m here7.1"<<std::endl;
        // waypoint.heading = current_pose.pose.position.heading;
        //this is pblm look into it
        waypoint.heading = 0;
            std::cout<<"m here7.2"<<std::endl;
        visited_waypoint_vector.push_back(waypoint);
            std::cout<<"m here7.3"<<std::endl;
        Pathreq.path.points.push_back(waypoint);
            std::cout<<"m here7.4"<<std::endl;
    }
            std::cout<<"m here8"<<std::endl;
...
void boundry_detection_node_class::TFBroadcaster(){
